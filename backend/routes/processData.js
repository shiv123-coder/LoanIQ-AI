...
const { processLimiter } = require('../middleware/rateLimiter');

/**
 * POST /process-data
 * Main loan processing endpoint
 */
const failedAttempts = new Map(); // Store IP fraud attempts

router.post('/process-data', processLimiter, async (req, res) => {
  try {
    const ip = req.ip || req.connection.remoteAddress || 'unknown';
    if (failedAttempts.get(ip) >= 3) {
      console.warn(`🚨 SECURITY ALERT: IP ${ip} blocked due to multiple fraud/OCR failures.`);
      return res.status(403).json({ 
        success: false, 
        error: 'SECURITY ALERT: IP locked due to multiple suspicious activities or failed verifications. Contact support.' 
      });
    }
    ...
    // ─── STEP 5: Decision ────────────────────────────────────────────────────
    let decision = getDecision(creditScore);
    console.log('✅ Initial Decision:', decision);

    // 🌟 RAZORPAY GRACEFUL FAILURE ROUTING 🌟
    // If OCR failed entirely (e.g. blurry image or service outage), do NOT reject or crash.
    // Gracefully route to manual review.
    if (panDetails.error || panDetails.panIssue === 'LOW_QUALITY' || panDetails.panIssue === 'FAKE_DETECTED') {
      decision = 'MANUAL_REVIEW_REQUIRED';
      
      // 🛡️ SECURITY FALLBACK: Log suspicious activity and increment IP strike
      const currentStrikes = (failedAttempts.get(ip) || 0) + 1;
      failedAttempts.set(ip, currentStrikes);
      console.log(`⚠️ Graceful Failure Triggered: Routing to MANUAL_REVIEW_REQUIRED. Security Strike for IP ${ip}: ${currentStrikes}/3`);
      
      if (userId) {
        const { createNotification } = require('../services/dbService');
        await createNotification('admin', null, 'Security Alert', `Suspicious document upload detected from IP ${ip}.`, 'security', userId, 'security_alert');
      }
    } else {
      // Reset strikes on successful genuine OCR
    ...
    const report = {
      ...
      creditScore,
      auditTrail: adjustments, // Export explicitly as auditTrail for Razorpay explainability
      adjustments,
      riskScore,
    ...
    if (docId) {
      const { createNotification } = require('../services/dbService');
      await createNotification('admin', null, 'New Loan Application', `A new loan application (${docId}) was submitted.`, 'application', docId, 'loan_application');
      await createNotification('loan_officer', null, 'New Loan Application', `A new loan application (${docId}) requires review.`, 'application', docId, 'loan_application');
    }
