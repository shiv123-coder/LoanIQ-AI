...
function extractName(text) {
  ...
  // ── English patterns ──────────────────────────────────────────────────────
  const enPatterns = [
    // "my name is Shivshankar Mali Sharma" / "my name is Shivshankar Mali"
    /my\s+name\s+is\s+([A-Za-z][A-Za-z\s]{1,35}?)(?:\s*[.,]|\s+and\s|\s+my\s|\s+I\s|\s+i\s|$)/i,
    // "I am Shivshankar Mali" / "I'm Shivshankar Mali"
    /I(?:'m|\s+am)\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)?)/,
    // "name: Shivshankar Mali" / "name Shivshankar Mali"
    /name\s*[:]\s*([A-Za-z][A-Za-z\s]{2,30})/i,
    // "this is Shivshankar Mali speaking" / "this is Shivshankar Mali"
    /this\s+is\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)?)/,
    // "call me Shivshankar Mali"
    /call\s+me\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)?)/i,
  ];

  // ── Hindi (transliterated) patterns ───────────────────────────────────────
  const hiPatterns = [
    // "mera naam Shivshankar Mali hai" / "mera naam Shivshankar Mali"
    /mera\s+naam\s+([A-Za-z][A-Za-z\s]{1,30}?)(?:\s+hai|\s*[.,]|\s+aur\s|\s+meri\s|\s+mera\s|$)/i,
    // "mai Shivshankar Mali hun" / "main Shivshankar Mali hoon"
    /mai[n]?\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)?)\s*(?:hu[n]?|hoo[n]?)?/i,
    // "naam hai Shivshankar Mali"
  ...
  const mrPatterns = [
    // "majhe naav Shivshankar Mali aahe" / "mazhe naav Shivshankar Mali"
    /ma[jz]h?e?\s+na[av]+\s+([A-Za-z][A-Za-z\s]{1,30}?)(?:\s+(?:aahe|ahe)|\s*[.,]|\s+(?:ani|aani|majhe|mazhe)|$)/i,
    // "mi Shivshankar Mali" / "mee Shivshankar Mali"
    /m[ie]{1,2}\s+([A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)?)\s*(?:aahe|ahe)?/i,
    // "naav aahe Shivshankar Mali"
