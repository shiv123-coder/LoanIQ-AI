...
/**
 * Extracts the actual values/answers from the transcript for UI feedback.
 */
export function extractAnswerValues(transcript: string): Partial<Record<QuestionId, string>> {
  if (!transcript) return {};
  const lower = transcript.toLowerCase();
  const result: Partial<Record<QuestionId, string>> = {};

  // 1. Name
  const nameMatch = transcript.match(/(?:my name is|i am|mera naam|maje nav|myself)[\s,.]+([A-Z][a-zA-Z\s]*)/i);
  if (nameMatch && nameMatch[1]) {
    result.name = nameMatch[1].trim().split(/\s+/).slice(0, 3).join(' '); // Up to 3 words
  }

  // 2. Income
  const numMatch = transcript.match(/(?:₹|rs\.?\s*)?(\d[\d,]*)\s*(lakh|lac|thousand|k|हजार|लाख)?/i);
  if (isQuestionAnswered('income', transcript)) {
    if (numMatch) result.income = `₹${numMatch[1]} ${numMatch[2] || ''}`.trim();
  }

  // 3. Amount
  if (isQuestionAnswered('amount', transcript)) {
    const amountMatch = transcript.match(/(?:i need|require|want|loan of)[\s\w]*(?:₹|rs\.?\s*)?(\d[\d,]*)\s*(lakh|lac|thousand|k|हजार|लाख)?/i);
    if (amountMatch) result.amount = `₹${amountMatch[1]} ${amountMatch[2] || ''}`.trim();
    else if (numMatch && !result.income) result.amount = `₹${numMatch[1]} ${numMatch[2] || ''}`.trim(); // fallback if income wasn't matched
  }

  // 4. Job Type
  if (/(salaried|employee|job|नौकरी)/i.test(transcript)) result.jobType = 'Salaried';
  else if (/(self-employed|business|freelancer|व्यवसाय|दुकान)/i.test(transcript)) result.jobType = 'Self-Employed';

  // 5. Purpose
  if (/(home|house|flat)/i.test(transcript)) result.purpose = 'Home Loan';
  else if (/(car|bike|vehicle)/i.test(transcript)) result.purpose = 'Vehicle Loan';
  else if (/(education|study|college)/i.test(transcript)) result.purpose = 'Education Loan';
  else if (/(medical|hospital)/i.test(transcript)) result.purpose = 'Medical Emergency';
  else if (/(business|shop|startup)/i.test(transcript)) result.purpose = 'Business Loan';
  else if (/(pc|computer|laptop|gaming)/i.test(transcript)) result.purpose = 'Electronics/PC';
  else if (isQuestionAnswered('purpose', transcript)) result.purpose = 'Personal/Other';

  // 6. Age
  const ageMatch = transcript.match(/(\d{2})\s*(years|saal|varsh|old)/i);
  if (ageMatch) result.age = `${ageMatch[1]} Years`;
