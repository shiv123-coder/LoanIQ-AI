...
function fmtDate(iso?: string) {
  return (iso ? new Date(iso) : new Date()).toLocaleDateString("en-IN", {
    day: "2-digit",
    month: "long",
    year: "numeric",
  });
...
async function getLocationText(location: any): Promise<string> {
  try {
    ...
    const r = await fetch(
      ...
      { headers: { "Accept-Language": "en", "User-Agent": "LoanIQ /1.0" } },
    );
...
export async function downloadLoanSanctionPDF(
  ...
  const [locText, qrData] = await Promise.all([
    getLocationText(cust.location),
    makeQR(appUrl),
  ]);
  ...
  doc.circle(ML + 7, 13, 6, "F");
  setT("bold", 9, BLU);
  doc.text("CS", ML + 3.8, 14.5);
  setT("bold", 14, WHT);
  doc.text("LoanIQ ", ML + 17, 11);
  setT("normal", 6.5, [200, 225, 255] as C3);
  doc.text(
    "Intelligent Credit Solutions  |  NBFC Reg. No. RBI-2024-NBFC-0042",
    ML + 17,
    17,
  );
  setT("normal", 7, WHT);
  ...
  // ── TITLE ──────────────────────────────────────────────────────────
  fillR(ML, y, CW, 10, LBLU);
  bordR(ML, y, CW, 10, BLU, 0.4);
  fillR(ML, y, 3, 10, BLU);
  setT("bold", 11, BLU);
  doc.text("LOAN SANCTION LETTER", PW / 2, y + 6.8, { align: "center" });
  y += 13;
  ...
  setT("bold", 7, BLU);
  doc.text("APPLICANT DETAILS", C1X + 5, BOXY + 5.5);
  ...
  setT("bold", 7, WHT);
  doc.text("SANCTIONED AMOUNT", C2X + 3, BOXY + 4.5);

  const amtStr = fmt(offer.offeredAmount || 0);
  setT("bold", 16, GRN);
  doc.text(amtStr, C2X + C2W / 2, BOXY + 18, { align: "center" });
  setT("normal", 6, MUT);
  doc.text("Amount Approved", C2X + C2W / 2, BOXY + 23, { align: "center" });
  ...
  setT("normal", 6.5, MUT);
  doc.text("Account No:", C2X + 4, ry2 + 1.5);
  ...
  setT("bold", 7, BLU);
  doc.text("CREDIT ASSESSMENT", SC_X + 5, y + 5);
  ...
  setT("bold", 13, barCol);
  doc.text(`${score}`, SC_X + 6, barY + 11);
  setT("normal", 8, MUT);
  doc.text("/900", SC_X + 6 + 14, barY + 11);

  setT("normal", 6.5, MUT);
  doc.text(
    `Risk Level: ${riskLevel}  |  Band: ${scoreBand}`,
    SC_X + 6,
    barY + 17,
  );
  doc.text(
    `Decision: ${decision}  |  Risk Score: ${riskScore}/100`,
    SC_X + 6,
    barY + 22,
  );
  ...
  setT("bold", 7, GRN);
  doc.text("VERIFICATION STATUS", VE_X + 5, y + 5);
  ...
  verItems.forEach((item) => {
    ...
    setT("bold", 7.5, col);
    doc.text(item.ok ? "[PASS]" : "[FAIL]", VE_X + 5, vly);
  ...
  setT("bold", 7.5, WHT);
  doc.text("CREDIT SCORING FACTORS", ML + 3, y + 4.5);
  ...
  if (adjs.length > MAX_ROWS) {
    setT("italic", 6, MUT);
    doc.text(`  + ${adjs.length - MAX_ROWS} more factors`, ML + 3, y + 3.5);
    y += 5;
  }
  ...
  if (qrData) {
    ...
    setT("bold", 5.5, BLU);
    doc.text("TRACK REPAYMENT", QR_X + QR_W / 2, qrImgY + qrImgSize + 4, {
      align: "center",
    });
    setT("normal", 5, MUT);
    doc.text("Given vs Paid Status", QR_X + QR_W / 2, qrImgY + qrImgSize + 8, {
      align: "center",
    });
  }
  ...
  setT("bold", 7, BLU);
  doc.text("KEY TERMS & CONDITIONS", ML + 5, y + 5.5);
  ...
  setT("normal", 6.5, MUT);
  doc.text("Borrower Acceptance", ML + 3, y + 5);
  ...
  setT("normal", 6.5, MUT);
  doc.text("Authorised Signatory", S2X + 3, y + 5);
  hLine(S2X + 3, y + 15, S2X + SIG_W - 3, DRK, 0.5);
  setT("bold", 7, BLU);
  doc.text("LoanIQ  Financial Services", S2X + 3, y + 19);
  ...
  setT("normal", 6, WHT);
  doc.text(`LoanIQ   |  Ref: ${ref}  |  CONFIDENTIAL`, ML, PH - 3.8);
