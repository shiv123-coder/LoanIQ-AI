...
import { detectAnsweredQuestions, extractAnswerValues } from '../utils/nlpMatcher';
...
export default function VideoKYC() {
  const { lang } = useLang();
  const { t } = useTranslation(['interview', 'common', 'kyc']);
  ...
  const [extractedAnswers, setExtractedAnswers] = useState<Partial<Record<string, string>>>({});
    ...
    if (panImage && phase === 'pan' && !ocrData) {
      const fetchOcr = async () => {
        ...
        try {
          ...
          if (!res.ok) throw new Error(t('kyc:errorExtractPan', 'Failed to extract PAN details'));
          ...
          setOcrError(err.message || t('kyc:errorOcr', 'OCR extraction failed'));
  ...
  // Detect which questions have been answered based on transcript
  useEffect(() => {
    const fullText = (transcript + ' ' + currentTranscript).trim();
    const newAnswered = detectAnsweredQuestions(fullText);
    if (JSON.stringify(newAnswered.sort()) !== JSON.stringify([...answeredQuestions].sort())) {
      setAnsweredQuestions(newAnswered);
    }
    setExtractedAnswers(extractAnswerValues(fullText));
  ...
  async function startCamera() {
    try {
      ...
      setError(t('kyc:errorCameraAccess', 'Camera access failed. Please check permissions.'));
      return;
    }
  ...
  function startListening() {
    ...
    if (!SpeechRecognition) {
      setError(t('kyc:errorSpeech', 'Speech recognition not supported. Please type your answer.'));
      return;
    }
    ...
    recognition.onend = () => {
      ...
      if (recognitionRef.current === recognition) {
        // Still supposed to be listening — restart immediately after a tiny delay
        setTimeout(() => {
          if (recognitionRef.current === recognition) {
            try { 
              recognition.start(); 
    ...
    recognition.onerror = (e: any) => {
      ...
      // If it's a real error, show it to the user so we can debug!
    ...
    recognition.onresult = (e: any) => {
      ...
      // Clean handling of interim vs final text
      if (interimText) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="rounded-2xl overflow-hidden bg-card border border-border aspect-[3/4] relative">
              ...
              <div className="absolute bottom-3 left-3 right-3">
                <div className="flex items-center gap-2 bg-black/50 backdrop-blur-sm rounded-xl px-3 py-2">
                  ...
                  <span className="text-white-always text-xs font-medium">{t('kyc:cameraActive', 'Camera Active')}</span>
                </div>
              </div>
              ...
              {isScanningAge && (
                <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/20 backdrop-blur-[2px]">
                  <div className="bg-card/90 p-4 rounded-2xl border border-primary/30 flex flex-col items-center gap-3 animate-in fade-in zoom-in duration-300">
                    ...
                    <span className="text-xs font-bold text-primary">{t('kyc:calibratingAi', 'CALIBRATING WITH REAL AI...')}</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="lg:col-span-3 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[t('kyc:stepLiveness', 'Liveness'), t('kyc:stepQuestions', 'Questions'), t('kyc:stepPan', 'PAN Upload')].map((step, i) => {
              ...
              <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
                ...
                <div className="space-y-3 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                  {QUESTIONS.map((q, idx) => {
                    ...
                    const answerText = extractedAnswers[q.id];
                    return (
                      <div key={q.id} className={`transition-all duration-500 rounded-xl p-3 border ${
                        ...
                        {isAnswered && answerText && (
                          <div className="mt-2 pt-2 border-t border-emerald-500/20 flex items-center justify-between animate-in fade-in slide-in-from-top-1 duration-300">
                            <span className="text-xs text-emerald-600/80 uppercase font-bold">{t('interview:answer', 'Answer')}:</span>
                            <span className="text-sm font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-md border border-emerald-500/20">{answerText}</span>
                          </div>
                        )}
                      </div>
                    );
              ...
              <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
                ...
                {!panImage ? (
                  <div className="p-6 rounded-2xl border-2 border-dashed border-primary/30 bg-primary/5 flex flex-col items-center justify-center gap-4 text-center animate-pulse">
                    ...
                    <div>
                      <p className="text-sm font-bold text-foreground">{t('kyc:autoCaptureActive', 'AUTO-CAPTURE ACTIVE')}</p>
                      <p className="text-xs text-muted-foreground mt-1">{t('kyc:showPanCard', 'Please show your PAN card to the camera. Countdown starts automatically.')}</p>
                    </div>
                  </div>
                ) : (
                  <div className="relative group animate-in fade-in zoom-in duration-300">
                    <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-emerald-400 truncate">{t('kyc:capturedSuccessfully', 'Captured Successfully')}</p>
                        <p className="text-[10px] text-emerald-500/60 uppercase font-bold">{t('kyc:ocrAnalysisReady', 'OCR ANALYSIS READY')}</p>
                      </div>
                      <button 
                      ...
                      >
                        <X size={16} />
                        <span className="text-[8px] font-bold">{t('kyc:retake', 'RETAKE')}</span>
                      </button>
                    </div>
                  </div>
                )}
                ...
                {panImage && (
                  <div className="bg-secondary/30 border border-border rounded-xl p-4 space-y-3 mt-4">
                    <p className="text-xs font-semibold text-muted-foreground uppercase">{t('kyc:extractedDetails', 'Extracted Details (Verify & Edit)')}</p>
                    {isOcrLoading ? (
                      <div className="flex items-center gap-2 text-sm text-primary animate-pulse">
                        <div className="w-4 h-4 border-2 border-primary border-t-transparent rounded-full animate-spin" />
                        {t('kyc:scanningDoc', 'Scanning document...')}
                      </div>
                    ) : ocrError ? (
                      ...
                      <div className="space-y-2">
                        <input
                          ...
                          placeholder={t('kyc:panNumber', 'PAN Number')}
                        ...
                        <input
                          ...
                          placeholder={t('kyc:fullName', 'Full Name')}
                          className="w-full bg-background border border-border focus:border-primary rounded-lg px-3 py-2 text-sm text-foreground outline-none transition-colors uppercase"
                        />
                        <input
                          type="text"
                          ...
                          placeholder={t('kyc:dob', 'Date of Birth (DD/MM/YYYY)')}
                ...
                <div className="space-y-3">
                  ...
                  >
                    {isProcessing ? (
                      ...
                      <><CheckCircle size={18} /> {panImage ? t('submitAndAnalyze') : t('kyc:waitingForId', 'Waiting for ID...')}</>
                    )}
                  </button>
