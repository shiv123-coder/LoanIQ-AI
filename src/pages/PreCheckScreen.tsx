...
export default function PreCheckScreen() {
  ...
  async function runChecks() {
    ...
    try {
      ...
      if (conn) {
        ...
        if (type === "4g") {
          quality = "Good";
          detail = t('precheck:network4g', '4G — Excellent');
        } else if (type === "3g") {
          quality = "Medium";
          detail = t('precheck:network3g', '3G — Adequate');
        } else {
          quality = "Poor";
          detail = t('precheck:networkSlowType', '{{type}} — Slow', { type });
        }
      } else {
        ...
        if (ms < 300) {
          quality = "Good";
          detail = t('precheck:networkFast', '{{ms}}ms — Fast', { ms });
        } else if (ms < 800) {
          quality = "Medium";
          detail = t('precheck:networkModerate', '{{ms}}ms — Moderate', { ms });
        } else {
          quality = "Poor";
          detail = t('precheck:networkSlow', '{{ms}}ms — Slow', { ms });
        }
      }
      const status =
        quality === "Good"
      ...
      updateCheck("network", {
        status: "warning",
        detail: t('precheck:networkFailed', 'Could not measure'),
      });
    }
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          updateCheck("location", {
            status: "ready",
            detail: t("locationEnabled"),
          });
          setState((prev) => ({
            ...prev,
  ...
  const canProceed =
    checks.camera.status === "ready" && checks.mic.status === "ready";
  const allDone = Object.values(checks).every((c) => c.status !== "checking");

  function statusIcon(status: CheckStatus) {
    if (status === "checking")
      return (
        <div className="w-6 h-6 border-[3px] border-slate-200 dark:border-white/10 !border-t-blue-600 dark:!border-t-primary rounded-full animate-spin" />
      );
    if (status === "ready")
      return <CheckCircle size={20} className="text-emerald-400" />;
    if (status === "warning")
      return (
        <div className="flex items-center gap-2 text-amber-400 font-semibold text-xs border border-amber-500/30 px-2 py-1 rounded-lg bg-amber-500/10">
          <AlertCircle size={14} />
          {t("retry")}
        </div>
      );
    return (
      <div className="flex items-center gap-2 text-red-400 font-semibold text-xs border border-red-500/30 px-2 py-1 rounded-lg bg-red-500/10">
        <XCircle size={14} />
        {t("retry")}
      </div>
    );
        ...
        {!isAuth && (
          <div className="mt-12 pt-6 border-t border-border flex flex-col items-center gap-6">
            ...
            <p className="text-muted-foreground text-[10px] uppercase tracking-widest font-medium">
              {t("common:poweredBy", "Powered by")} <span className="text-blue-500/80">LoanIQ </span> ·{" "}
              {t("secure", "Secure")}
            </p>
          </div>
        )}
