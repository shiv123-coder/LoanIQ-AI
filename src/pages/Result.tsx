...
export default function Result() {
  ...
  const decisionConfig = {
    ...
    CONDITIONAL: {
      ...
      label: t('result:pendingReview', 'Application Pending Review'),
      sub: t('result:pendingReviewSub', 'Your credit score (491–750) qualifies you for manual review. An admin will approve or reject your loan shortly.'),
    },
    REJECTED: {
      color: 'text-red-400',
      ...
      <div className="relative max-w-2xl mx-auto space-y-5">
        ...
        {report?.adjustments && report.adjustments.length > 0 && (
          <div className="bg-card border border-border rounded-2xl p-5">
            <div className="text-muted-foreground text-xs font-medium mb-4 uppercase tracking-wider flex items-center gap-2">
              ...
              {t('result:pointBreakdown', 'Point System Breakdown')}
            </div>
            <div className="space-y-2">
              ...
              <div className="pt-2 mt-2 border-t border-border flex justify-between items-center px-3">
                <span className="text-foreground font-semibold">{t('result:finalScore', 'Final Credit Score')}</span>
        ...
        {report?.panDetails && (report.panDetails.panNumber || report.panDetails.nameMatch || report.panDetails.quality) && (
          <div className="bg-card border border-border rounded-2xl p-5">
            <div className="text-muted-foreground text-xs font-medium mb-4 uppercase tracking-wider flex items-center gap-2">
              ...
              {t('result:panVerification', 'PAN Card Verification & Identity Match')}
            </div>
            ...
            <div className="space-y-3">
              ...
              {report.panDetails.panNumber && (
                <div className="flex items-center justify-between p-3 bg-secondary/50 rounded-xl">
                  <span className="text-muted-foreground text-xs">{t('result:panNumber', 'PAN Number')}</span>
              ...
              {report.panDetails.nameMatch && report.panDetails.nameMatch.panName && (
                <>
                  <div className="flex items-center justify-between p-3 bg-secondary/50 rounded-xl">
                    <span className="text-muted-foreground text-xs">{t('result:nameOnPan', 'Name on PAN Card')}</span>
                    <span className="text-foreground text-sm font-medium">{report.panDetails.nameMatch.panName}</span>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-secondary/50 rounded-xl">
                    <span className="text-muted-foreground text-xs">{t('result:nameInInterview', 'Name Stated in Interview')}</span>
                    <span className="text-foreground text-sm font-medium">{report.panDetails.nameMatch.spokenName || '—'}</span>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-secondary/50 rounded-xl">
                    <span className="text-muted-foreground text-xs">{t('result:nameMatchScore', 'Name Match Score')}</span>
              ...
              {report.panDetails.quality && (
                <div className="flex items-center justify-between p-3 bg-secondary/60 rounded-xl">
                  <span className="text-muted-foreground text-xs">{t('result:imageQuality', 'Image Quality')}</span>
                  <div className="flex items-center gap-2">
              ...
              {report.panDetails.panIssue && (
                <div className="p-3 bg-red-500/5 border border-red-500/20 rounded-xl">
                  <div className="text-red-300 text-xs font-semibold mb-1">{t('result:issueDetected', '⚠️ Issue Detected')}</div>
        ...
        {report?.ageAnalysis && (
          <div className="bg-card border border-border rounded-2xl p-5">
            <div className="text-muted-foreground text-xs font-medium mb-4 uppercase tracking-wider flex items-center gap-2">
              ...
              {t('result:aiAgeVerification', 'AI Age Verification')}
            </div>
            
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-secondary/40 p-3 rounded-xl border border-border/50 text-center">
                <div className="text-muted-foreground text-[10px] uppercase font-bold mb-1">{t('result:statedUser', 'Stated (User)')}</div>
                <div className="text-foreground text-xl font-bold">{report.ageAnalysis.statedAge || '—'}</div>
                <div className="text-[9px] text-muted-foreground mt-1">{t('result:fromInterview', 'From Interview')}</div>
              </div>
              <div className="bg-secondary/40 p-3 rounded-xl border border-border/50 text-center">
                <div className="text-muted-foreground text-[10px] uppercase font-bold mb-1">{t('result:identityId', 'Identity (ID)')}</div>
                <div className="text-foreground text-xl font-bold">{report.ageAnalysis.idAge || '—'}</div>
                <div className="text-[9px] text-muted-foreground mt-1">{t('result:fromPanOcr', 'From PAN OCR')}</div>
              </div>
              <div className="bg-secondary/40 p-3 rounded-xl border border-border/50 text-center flex flex-col items-center justify-center">
                <div className="text-muted-foreground text-[10px] uppercase font-bold mb-1">{t('result:aiInference', 'AI Inference')}</div>
                {report.ageAnalysis.aiModelConnected ? (
                  <>
                    <div className="text-foreground text-xl font-bold">{report.ageAnalysis.realAiAge || '—'}</div>
                    <div className="text-[9px] text-emerald-400 mt-1 font-bold">{t('result:confidence', 'Confidence:')} {report.ageAnalysis.realAiConfidence}%</div>
                  </>
                ) : (
                  <div className="text-red-400 text-[10px] font-bold leading-tight px-1">
                    {t('result:noAiModel', 'No AI model connected')}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mt-4 space-y-2">
              {report.ageAnalysis.realAiAge && report.ageAnalysis.idAge && (
                <div className="p-2 bg-secondary/20 rounded-lg flex items-center justify-between">
                  <span className="text-[10px] text-muted-foreground uppercase font-bold">{t('result:aiVsId', 'AI vs ID')}</span>
                  <div className="text-xs font-bold">
                    {Math.abs(report.ageAnalysis.realAiAge - report.ageAnalysis.idAge) <= 5 ? (
                      <span className="text-emerald-400 flex items-center gap-1"><CheckCircle size={12}/> {t('result:verified', 'Verified')}</span>
                    ) : (
                      <span className="text-red-400 flex items-center gap-1"><XCircle size={12}/> {t('result:mismatch', 'Mismatch')}</span>
                    )}
                  </div>
                </div>
              )}
              ...
              {report.ageAnalysis.statedAge && report.ageAnalysis.idAge && (
                <div className="p-2 bg-secondary/20 rounded-lg flex items-center justify-between">
                  <span className="text-[10px] text-muted-foreground uppercase font-bold">{t('result:statedVsId', 'Stated vs ID')}</span>
                  <div className="text-xs font-bold">
              ...
              {report.ageAnalysis.realAiAge && report.ageAnalysis.statedAge && (
                <div className="p-2 bg-secondary/20 rounded-lg flex items-center justify-between">
                  <span className="text-[10px] text-muted-foreground uppercase font-bold">{t('result:interviewVsAi', 'Interview vs AI')}</span>
                  <div className="text-xs font-bold">
                    {Math.abs(report.ageAnalysis.realAiAge - report.ageAnalysis.statedAge) <= 10 ? (
                      <span className="text-emerald-400 flex items-center gap-1"><CheckCircle size={12}/> {t('result:consistent', 'Consistent')}</span>
                    ) : (
                      <span className="text-red-400 flex items-center gap-1"><XCircle size={12}/> {t('result:inconsistent', 'Inconsistent')}</span>
                    )}
                  </div>
                </div>
              )}
          ...
          <div className="bg-destructive/10 border border-destructive/30 rounded-2xl p-5 space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-muted-foreground text-xs mt-0.5">
                  {t('result:rejectionSub', 'Specific issues found with your application')}
                </div>
              </div>
            </div>
            ...
            {report?.rejectionReasons && report.rejectionReasons.length > 0 ? (
              ...
              <div className="bg-card border border-border rounded-xl p-4 flex gap-3 items-start">
                ...
                <div>
                  <h4 className="text-foreground font-semibold text-sm">{t('result:defaultRejectionTitle', 'Application Did Not Meet Criteria')}</h4>
                  <p className="text-muted-foreground text-xs mt-1 leading-relaxed">
                    {t('result:defaultRejectionDesc', 'Your application did not meet our eligibility criteria. Please ensure you provide a valid PAN card, state your income clearly in the interview, and complete the liveness check.')}
                  </p>
                </div>
              </div>
            )}
        ...
        {decision === 'CONDITIONAL' && (
          <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl p-4 flex gap-4 items-start">
            ...
            <div>
              <h4 className="text-amber-500 font-semibold text-sm">{t('result:pendingAdminTitle', '⏳ Pending Admin Review')}</h4>
              <p className="text-muted-foreground text-xs mt-1 leading-relaxed">
                {t('result:pendingAdminDesc', 'Your application has been submitted and is awaiting review by our loan officer. Your credit score (491–750) requires manual verification. You will be notified once a decision is made.')}
              </p>
            </div>
          </div>
        )}
