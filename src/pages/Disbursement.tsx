...
export default function Disbursement() {
  ...
  function validate() {
    ...
    if (!accountNumber.trim() || accountNumber.length < 9 || accountNumber.length > 18) {
      e.account = t('disbursement:errorAccount', 'Please enter a valid account number (9–18 digits)');
    }
    if (!/^[A-Z]{4}0[A-Z0-9]{6}$/.test(ifscCode.toUpperCase())) {
      e.ifsc = t('disbursement:errorIfsc', 'Invalid IFSC code format (e.g. SBIN0001234)');
    }
    setErrors(e);
  ...
  async function handleDownloadPDF() {
    ...
    try {
      ...
      alert(t('disbursement:pdfError', 'Could not generate PDF. Please try again.'));
    } finally {
      setPdfLoading(false);
    }
  }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        ...
        <div className="relative max-w-md w-full text-center">
          ...
          <h2 className="text-3xl font-bold text-foreground mb-2">{t('disbursement:amountCredited', 'Amount Credited!')}</h2>
          <p className="text-muted-foreground mb-6">{t('disbursement:disbursedDesc', 'Your loan has been successfully disbursed')}</p>
          ...
          <div className="bg-gradient-to-br from-emerald-900/40 to-teal-900/30 border border-emerald-500/30 rounded-2xl p-6 mb-6 space-y-4">
            ...
            <div className="text-foreground/80">{t('disbursement:creditedTo', 'credited to')}</div>
            <div className="bg-secondary/60 rounded-xl px-4 py-3 border border-border">
              <div className="text-muted-foreground text-xs">{t('disbursement:account', 'Account')}</div>
              <div className="text-foreground font-mono font-semibold text-lg">{maskAccount(accountNumber)}</div>
              <div className="text-muted-foreground text-xs mt-1">{t('disbursement:ifsc', 'IFSC:')} {ifscCode.toUpperCase()}</div>
            </div>

            <div className="grid grid-cols-2 gap-3 pt-2">
              <div className="bg-secondary/40 rounded-xl p-3 border border-border">
                <div className="text-muted-foreground text-xs">{t('disbursement:monthlyEmi', 'Monthly EMI')}</div>
                <div className="text-foreground font-semibold">₹{offer.emi.toLocaleString('en-IN')}</div>
              </div>
              <div className="bg-secondary/40 rounded-xl p-3 border border-border">
                <div className="text-muted-foreground text-xs">{t('disbursement:interest', 'Interest')}</div>
                <div className="text-foreground font-semibold">{offer.interestRate}% {t('disbursement:pa', 'p.a.')}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-primary/10 border border-primary/20 rounded-xl p-4 mb-6 text-left">
            <p className="text-primary text-sm">
              {t('disbursement:dear', 'Dear')} <span className="font-semibold">{name}</span>, {t('disbursement:yourEmiOf', 'your EMI of')}{' '}
              <span className="font-semibold text-foreground">₹{offer.emi.toLocaleString('en-IN')}</span> {t('disbursement:willStart', 'will start from next month. Please ensure sufficient balance in your account.')}
            </p>
          </div>
          ...
          <div className="space-y-3">
            ...
            >
              {pdfLoading ? (
                <><div className="w-5 h-5 border-[3px] border-white/30 border-t-white rounded-full animate-spin" />{t('disbursement:generatingDoc', 'Generating Document…')}</>
              ) : (
                <><Download size={18} />{t('disbursement:downloadPdf', 'Download Official Sanction Letter (PDF)')}</>
              )}
            </button>
            ...
            >
              <FileText size={16} />
              {t('disbursement:viewReport', 'View Full AI Credit Report')}
            </button>
            ...
            >
              {t('disbursement:returnHome', 'Return to Home')}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-background py-8 px-4">
      ...
      <div className="relative max-w-md mx-auto">
        ...
        >
          <ArrowLeft size={16} />
          {t('disbursement:backToResult', 'Back to Result')}
        </button>
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-foreground mb-2">{t('disbursement:title', 'Loan Disbursement')}</h1>
          <p className="text-muted-foreground text-sm">{t('disbursement:subtitle', 'Enter your bank details to receive your loan')}</p>
        </div>
        ...
        {offer && (
          <div className="bg-gradient-to-r from-emerald-900/30 to-teal-900/20 border border-emerald-500/20 rounded-2xl p-5 mb-6">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <div className="text-muted-foreground text-xs">{t('disbursement:approvedAmount', 'Approved Amount')}</div>
                <div className="text-emerald-500 text-2xl font-bold">₹{offer.offeredAmount.toLocaleString('en-IN')}</div>
              </div>
              <div>
                <div className="text-muted-foreground text-xs">{t('disbursement:rate', 'Rate')}</div>
              ...
              <div>
                <div className="text-muted-foreground text-xs">{t('disbursement:tenure', 'Tenure')}</div>
                <div className="text-foreground font-semibold">{offer.tenure} {t('disbursement:months', 'months')}</div>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
          <div className="flex items-center gap-2 text-foreground font-semibold mb-2">
            ...
            {t('disbursement:bankDetails', 'Bank Account Details')}
          </div>

          {/* Account Number */}
          <div>
            <label className="block text-muted-foreground text-sm mb-1.5" htmlFor="account-number">
              {t('disbursement:accountNumber', 'Account Number')}
            </label>
            <input
              id="account-number"
              type="text"
              ...
              placeholder={t('disbursement:accountPlaceholder', 'Enter your bank account number')}
              maxLength={18}
          ...
          <div>
            <label className="block text-muted-foreground text-sm mb-1.5" htmlFor="ifsc-code">
              {t('disbursement:ifscCode', 'IFSC Code')}
            </label>
            <input
              id="ifsc-code"
              type="text"
              ...
              placeholder={t('disbursement:ifscPlaceholder', 'e.g. SBIN0001234')}
              maxLength={11}
          ...
          <div className="flex items-start gap-2 p-3 bg-blue-500/5 border border-blue-500/15 rounded-xl">
            ...
            <p className="text-blue-300/70 text-xs">{t('disbursement:securityNote', 'Your banking details are encrypted and secure. We use 256-bit SSL encryption.')}</p>
          </div>
          ...
          >
            {loading ? (
              <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />{t('disbursement:processing', 'Processing…')}</>
            ) : (
              <><CheckCircle size={18} />{t('disbursement:acceptDisburse', 'Accept Offer & Disburse')}</>
            )}
          </button>
        </div>
      </div>
